"""Dataset validation rules for the clean-data MVP."""
...
def validate_dataframe(data: pd.DataFrame) -> None:
    ...
    if missing_count:
        raise DatasetValidationError(
            ...
            "Missing-value handling is planned for a future version."
        )
...
def validate_problem(data: pd.DataFrame, target: str, task: str) -> None:
    ...
    unique_targets = target_values.nunique(dropna=False)
